...
1. 결제가 되면, 좌석이 배정된다.
1. 좌석이 배정되면 소속사에게 좌석과 고객 정보를 전달한다
1. 고객이 주문을 취소할 수 있다
1. 주문이 취소되면 배달이 취소된다
...
비기능적 요구사항
1. 트랜잭션
    1. 좌석 배치가 안된 예약건은 아예 결제가 성립되지 않아야 한다  Sync 호출 
1. 장애격리
    1. 좌석배정 기능이 수행되지 않더라도 예약은 365일 24시간 확인 할 수 있어야 한다  Async (event-driven), Eventual Consistency
